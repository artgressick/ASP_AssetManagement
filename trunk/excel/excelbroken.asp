...
<%
	...
	file_being_created= "AssetsBroken.xls"
	...
	if cint(request("idStatus")) = 1 then
		act.WriteLine("<th nowrap>Sent</th>")
	else
		act.WriteLine("<th nowrap>Returned</th>")
	end if
	act.WriteLine("</tr>")
	...
	if not rsInventory.EOF then
		do until rsInventory.EOF
			...
			if cint(request("idStatus")) = 1 then
				act.WriteLine("<td align=""right"">" & formatdatetime(rsInventory("dtSent"),2) & "</td>" )
			else
				act.WriteLine("<td align=""right"">" & formatdatetime(rsInventory("dtReturned"),2) & "</td>" )
			end if
			act.WriteLine("</tr>")
...
<table width="300" height="275" border="0" cellpadding="7" cellspacing="0">
  ...
  <tr>
    <td align="center"><font size="2" face="Arial, Helvetica, sans-serif"><strong>Excel
      File: <a href="AssetsBroken.xls">Assets Broken Report</a></strong></font></td>
  </tr>
</table>
</body>
</html>
