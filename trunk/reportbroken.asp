...
<%
	...
	if flag = 0 then
		...
		if session("idAccess") < "O" then
			sql = "execute ListInventoryBrokenReport " & idCustomer & "," & idStatus & "," & idWarehouse
		else
			sql = "execute ListInventoryBrokenReportbyAccess " & idCustomer & "," & idStatus & "," & idWarehouse & "," & session("idUser")
		end if
...
<form name="form1" method="post" action="reportbroken.asp">
            ...
            <td width="100%"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <tr> 
                  <td><table width="100%" border="0" cellspacing="0" cellpadding="3">
                      <tr> 
                        <td><font size="3" face="Arial, Helvetica, sans-serif"><strong>Broken Assets Report</strong></font></td>
                      </tr>
                    </table></td>
                </tr>
                <tr> 
...
%>
                              ...
                              <td align="left"><font size="2" face="Arial, Helvetica, sans-serif"> 
                                <select name="idStatus" size="1" id="idStatus">
                                  <option value="1" <%if cint(request("idStatus")) = 1 then%>selected<%end if%>>Broken</option>
                                  <option value="0" <%if cint(request("idStatus")) = 0 then%>selected<%end if%>>Repaired</option>
                                </select>
                                </font></td>
                      ...
                      <tr>
                        <td height="20" bgcolor="#c0c0c0" colspan="6">
                          <table width="100%" border="0" cellspacing="0" cellpadding="5">
                            <tr>
                              <td align="left"><a HREF="javascript:newWindow('excel/excelbroken.asp?idCustomer=<%=idCustomer%>&amp;idStatus=<%=idStatus%>&amp;idWarehouse=<%=idWarehouse%>')"><img SRC="images/exporttoexcel.gif" border="0" WIDTH="120" HEIGHT="19"></a></td>
                            </tr>
                          </table>
                        </td>
                      </tr>
...
<%
	if cint(request("idStatus")) = 1 then
%>
                        <td height="20" bgcolor="#6699cc"><font color="#FFFFFF" size="1" face="Arial, Helvetica, sans-serif">&nbsp;Sent</font></td>
<%
	else
%>
                        <td height="20" bgcolor="#6699cc"><font color="#FFFFFF" size="1" face="Arial, Helvetica, sans-serif">&nbsp;Returned</font></td>
<%
	end if
%>
                      </tr>
...
<%
		if cint(request("idStatus")) = 1 then
%>
                        <td height="20"><font size="1" face="Arial, Helvetica, sans-serif">&nbsp;<%=formatdatetime(rsInventory("dtSent"),2)%></font></td>
<%
		else
%>
                        <td height="20"><font size="1" face="Arial, Helvetica, sans-serif">&nbsp;<%=formatdatetime(rsInventory("dtReturned"),2)%></font></td>
<%
		end if
%>
                      </tr>
                      <tr bgcolor="#f5f5f5"> 
                        <td height="20" colspan="6"><font size="1" face="Arial, Helvetica, sans-serif">&nbsp;<%=trim(rsInventory("txtProblem"))%></font></td>
                      </tr>
                      <tr bgcolor="#c0c0c0"> 
