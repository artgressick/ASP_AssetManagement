...
<%
	...
	if session("idAccess") < "O" then
		...
		sql = "execute ListOpenOrdersbyUser " & session("idUser")
	end if
...
<form name="form1" method="post" action="retreivecart.asp">
                ...
                <tr> 
                  <td>
					<table width="100%" border="0" cellspacing="0" cellpadding="3">
                      <tr> 
                        <td width="50%"><font size="3" face="Arial, Helvetica, sans-serif"><strong>Retrieve Cart</strong></font></td>
                        <td width="50%">&nbsp;</td>
                      </tr>
                      <tr bgcolor="#f5f5f5"> 
...
%>
                      <tr> 
                        ...
                        <td height="20" align="right"><font size="1" face="Arial, Helvetica, sans-serif"><%if session("idAccess") < "P" and (rsCarts("idStatus") = 3 or rsCarts("idStatus") = 2) then%><a href="unlockcart.asp?idCart=<%=rsCarts("idCart")%>&idOrder=<%=idOrder%>">Unlock</a><%if session("idAccess") = "A" then%> | <a href="ordering/loadcart.asp?idCart=<%=rsCarts("idCart")%>">Open Cart</a><%end if%><%end if%><%if rsCarts("idStatus") = 1 then%><a href="ordering/loadcart.asp?idCart=<%=rsCarts("idCart")%>">Open Cart</a><%end if%></font></td>
                      </tr>
                      <tr bgcolor="#5b5b5b"> 
                        <td height="1" colspan="4"><img src="images/5b5b5bdot.gif" width="1" height="1"></td>
                      </tr>
