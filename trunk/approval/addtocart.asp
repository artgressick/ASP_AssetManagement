...
<%
	...
	if session("idAccess") < "O" then
		sql = "execute ListAvailablebyDescription " & request("idDescription") & "," & session("idCart")
	else
		sql = "execute ListAvailablebyDescriptionandAccess " & request("idDescription") & "," & session("idCart") & "," & session("idUser")
	end if
